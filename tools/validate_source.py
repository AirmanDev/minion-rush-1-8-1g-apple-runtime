...
def check_text_style(tree: dict[str, str]) -> None:
    forbidden = tuple(chr(code) for code in (0x2014, 0x2013, 0x2192, 0x2026,
                                              0x2212, 0x00B1, 0x2011))
    found = sorted(
        relative for relative, source in tree.items()
        if any(character in source for character in forbidden)
    )
    if found:
        raise SystemExit(
            "nonstandard punctuation remains: " + ", ".join(found)
        )

    accented = re.compile("[" + "\u00e1\u00e9\u00ed\u00f3\u00f6\u0151\u00fa\u00fc\u0171"
                          + "\u00c1\u00c9\u00cd\u00d3\u00d6\u0150\u00da\u00dc\u0170" + "]")
    words = (
        "hiany" + "zik", "hibas", "ervenytelen", "forras", "fajl",
        "jatek", "vendeg", "hasznalat", "kimenet", "konyvtar", "talalt",
        "futtasd", "serult", "csonkolt", "ures", "regi", "heti",
        "novekvo", "bizonyitek", "eredeti", "rendben", "bajt", "naplo",
        "betoltes", "fomenu", "kesz", "kiadas", "beallitas", "valtozo",
        "telepites", "torles", "pontosan", "duplikalt", "szabvanyos",
        "szukseges", "sikeres", "sikertelen", "leallt", "figyelem",
        "inditas", "letrehoz", "alkalmazas", "ablak", "gazda", "kepkocka",
        "ertek", "profil", "konfiguracio", "aktiv", "megjegyzes",
        "konstruktor", "statikus", "peldany", "szinkron", "utolso",
        "ismeretlen", "utasitas", "bejegyzes", "sztring", "terulet",
        "csatorna", "puffer", "szolgaltatas", "korrekcio", "kijelzo",
        "csokkent", "utem", "frissites", "memoria", "felderites",
        "jelentes", "fuggveny", "horog", "cimter", "ugrotabla",
        "kimaradas", "leghosszabb", "atlag", "felhasznaloi", "nema",
        "rajzolasi", "halozati", "alrendszer", "indult", "mozgaserzekelo",
        "ellenorzes", "nyomkovetes", "gyorsulasmero", "erintes",
        "elsimitas", "felulet", "tulajdonos", "elrejtve",
    )
    hungarian = re.compile(r"\b(?:" + "|".join(words) + r")\b", re.IGNORECASE)
    non_english = sorted(
        relative for relative, source in tree.items()
        if accented.search(source)
        or (relative != "tools/validate_source.py" and hungarian.search(source))
    )
    if non_english:
        raise SystemExit(
            "Hungarian text remains in maintained files: " + ", ".join(non_english)
        )

    prohibited_attribution = re.compile(
        r"\b(?:Chat" + "GPT|Co" + r"dex)\b|generated by (?:A" + "I|an artificial "
        + "intelligence)",
        re.IGNORECASE,
    )
    attributed = sorted(
        relative for relative, source in tree.items()
        if prohibited_attribution.search(source)
    )
    if attributed:
        raise SystemExit("prohibited attribution remains: " + ", ".join(attributed))
